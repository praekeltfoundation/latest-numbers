...
def retrieve_country_stats_from_arcgis(country_code):
    ...
    url = feature_server_url + f"/query?where=ISO_3_CODE+%3D+'{country_code}'&returnGeometry=false&outFields=NewCase,CumCase,NewDeath,CumDeath,date_epicrv,ISO_2_CODE&f=json"
    ...
    if latest["NewCase"] == 0 and latest["NewDeath"] == 0:
        ...
        # get vaccinations
        vaccinations = requests.get("https://covid19.who.int/who-data/vaccination-data.json").json()
